# -*- coding: utf-8 -*-
"""
Created on Mon Apr 13 16:28:03 2020
...
Tfi = 65 #Input initial fluid inlet temperature guess in celsius
Tfm_tar = 75 #Input the target mean fluid temperature
...
#Define variable to be used to sum the hourly gross incident radiation on the collector
It_col_gross_sum = 0
...
#Define list to append monthly gross incident radiation values to
It_col_gross_sum_list = []
...
#Define variable to sum ambient temperatures
Ta_tot = 0

#Create list to append average ambient temperatures to
Ta_list = []
...
for v in range(8760):
    ...
    def absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar):
        ...
        #Calculate total radaition incident on the collector's functional area
        It_col = Ibt_col+Idt_col+Igt_col
        
        #Calculate the total radiation incident on the collector's gross area
        It_col_gross = It_col*(Ag/Ac)
        ...
        Function_outputs = [Qu, It_col, Tfm, T_diff, theta_deg, It_col_gross]
        
        return Function_outputs

    #Output an array of useful power results
    Qu_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar)[0]
    ...
    Tfm_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar)[2]
    ...
    It_col_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar)[1]
    ...
    #Output array of total incident radiation on the plate
    It_col_gross_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar)[5]
    ...
    #Give each hourly incident gross radiation value in kilowatt hours
    It_col_gross_kW = It_col_gross_array/1000
    
    #Append the hourly energy output to the energy output list
    Qu_list.append(Qu_kW)
    
    #Sum hourly variable values so that monthly values can be obtained
    Qu_tot += Qu_kW
    
    It_col_sum += It_col_kW
    
    It_col_gross_sum += It_col_gross_kW
    ...
    #Append monthly variable values to the appropriate list
    if n == 31 and h == 24:
...
ws = wb["Chad_Athens_Comparison"] 

for i in range(len(Qu_tot_list)):

    #Define cell to write data to
    wcell1 = ws.cell(i+3,28)
    wcell2 = ws.cell(i+3,29)
    wcell3 = ws.cell(i+3,30)
    wcell4 = ws.cell(i+3,31)
