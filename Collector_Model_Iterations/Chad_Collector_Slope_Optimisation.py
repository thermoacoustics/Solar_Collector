...
beta_deg_list = np.linspace(0,30,100)
...
for v in range(8670):
    ...
    def absorbed_radiation(I,Ibh,Pg,beta_array,lat,n,h,t,a,Ac,Ag,Ta,m_dot,F_dash,P,Tfi,Ula,Ulb,Tfm_tar):
        ...
        theta = np.arccos((np.sin(delta)*np.sin(lat-beta_array))+(np.cos(delta)*np.cos(omega)*np.cos(lat-beta_array)))
        ...
        theta_tag = 90-(0.5788*beta_array)+(0.002693*(beta_array**2))
        #print('theta_tag:',theta_tag)
    
        #Calculate equivalent theta for diffuse radiation
        theta_tad = 59.7-(0.1388*beta_array)+(0.001497*(beta_array**2))
        ...
        #Calculate Rb
        Rb = ((np.cos(lat-beta_array))*(np.cos(delta))*(np.cos(omega))+(np.sin(lat-beta_array))*(np.sin(delta)))/((np.cos(lat))*(np.cos(delta))*(np.cos(omega))+(np.sin(lat))*(np.sin(delta)))
        #Rb = 1
        ...
        #Calculate the absorbed solar radiation per unit collector area
        S = Ibh*Rb*tab + Idh*tad*((1+np.cos(beta_array))/2) + I*Pg*tag*((1-np.cos(beta_array))/2)
        #print('S:',S)
    ...
    Qu_array = absorbed_radiation(I,Ibh,Pg,beta_array,lat,n,h,t,a,Ac,Ag,Ta,m_dot,F_dash,P,Tfi,Ula,Ulb,Tfm_tar)[0]
    ...
    It_col_array = absorbed_radiation(I,Ibh,Pg,beta_array,lat,n,h,t,a,Ac,Ag,Ta,m_dot,F_dash,P,Tfi,Ula,Ulb,Tfm_tar)[1]
...
for i in range(len(Qu_list)):
    ...
    wcell1 = ws.cell(i+3,4)
    wcell2 = ws.cell(i+3,3)
    wcell3 = ws.cell(i+3,2)
    #wcell4 = ws.cell(i+3,48)
    #wcell5 = ws.cell(i+2,53)

    #State what value should be written to the cell
    wcell1.value = Qu_list[i]
    wcell2.value = It_col_list[i]
    wcell3.value = beta_deg_list[i]
