# -*- coding: utf-8 -*-
"""
Created on Wed Apr 15 13:37:49 2020
...
P = 10*(10**5) #Input collector fluid pressure in bar
Fr = 0.8742
...
#Create a list of collector slopes to be tested
#beta_deg_list = np.linspace(0,50,50)

#Create a list of collector flow rates to be tested
m_dot_list = np.linspace(0.02,0.1,50)

#Convert collector slope list into an array
#beta_deg_array = np.array(beta_deg_list)

#Convert all slope array values to radians
#beta_array = beta_deg_array*np.pi/180

#print(beta_deg_list)
#print(beta_array)

#Create list to append energy values to for different collector slopes (other variables can be vaired as well)
Qu_beta_list = []
It_col_beta_list = []
Tfo_beta_list = []
Tfm_beta_list = []
...
Tfi = 90 #Input initial fluid inlet temperature guess in celsius
...
#Create list to append hourly collector output temperature values to
#Tfo_list = []
...
for u in range(len(m_dot_list)):
    ...
    #Create list to put all the hourly outlet temperatures in
    Tfo_list = []
    Tfm_list = []

    #Loop over the required input values
    for v in range(8760):
        I = ((ws1.cell(v+4,4).value)/3.6) #Read total radiation incident on a flat surface horizonatal in Watts per metre squared from Sceno weather data spreadsheet
        Ibh = ((ws1.cell(v+4,6).value)/3.6) #Input beam radiation incident on a flat surface perpendicular to the beam radiation in Watts per metre squared from Sceno weather data spreadsheet
        ...
        def absorbed_radiation(I,Ibh,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar):
            ...
            if I == 0 and Ibh == 0:
                ...
                Tfo = 0
                Tfm = 100
                Function_outputs = [Qu,It_col,Tfo,Tfm]
            ...
            #print(B_deg,delta_deg,omega_deg,theta_deg)
            ...
            F_dash = ((m_dot*Cp)/(Ac*Ul))*np.log(1/(1-(Ac*Ul*Fr)/(m_dot*Cp)))
        ...
        Qu_array = absorbed_radiation(I,Ibh,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar)[0]
    
        #Output an array of collector efficiency results
        #eff_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar)[2]

        #Output an array of the results for the temperature difference between the mean fluid temperature and ambient temperature
        #T_diff_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar)[3]

        #Output array for Ul results
        #Ul_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar)[4]

        #Output array of plate to fluid temperature difference results
        #T_grad_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar)[5]

        #Output array of fluid mean temperature
        Tfm_array = absorbed_radiation(I,Ibh,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar)[3]

        #Output array of plate mean temperature
        #Tpm_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar)[7]

        #Output array of total incident radiation on the plate
        It_col_array = absorbed_radiation(I,Ibh,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar)[1]

        #Output array of total beam radiation on the plate
        #Ibt_col_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar)[9]

        #Output array of total diffuse radiation on the plate
        #Idt_col_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar)[10]

        #Output array of total diffuse radiation on the plate
        #Igt_col_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar)[11]

        #Output array for the energy output for that hour
        #E_out_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar)[12]
    
        #Output array for angle of incidence for that hour
        #theta_deg_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar)[2]
    
        #Output array for collector output temperature
        Tfo_array = absorbed_radiation(I,Ibh,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Tfm_tar)[2]
        ...
        It_col_list.append(It_col_kW)
        
        Tfo_list.append(Tfo_array)
    ...
    for e in range(len(Qu_list)):
        Qu_all += Qu_list[e]
     
    It_all = 0    
        
    for c in range(len(It_col_list)):
        It_all += It_col_list[c]
    ...
    for bb in range(len(Tfo_list)):
        Tfo_all += Tfo_list[bb]
    
    Tfm_all = 0
    
    for cc in range(len(Tfm_list)):
        Tfm_all += Tfm_list[cc]

    #Append the collector slope energy values to the appropriate list
    Qu_beta_list.append(Qu_all)
    It_col_beta_list.append(It_all)
    Tfo_beta_list.append(Tfo_all/(365*24))
    Tfm_beta_list.append(Tfm_all/12)
...
ws = wb["Collector_Slope_Optimisation"] 

for i in range(len(Qu_beta_list)):

    #Define cell to write data to
    wcell1 = ws.cell(i+3,30)
    wcell2 = ws.cell(i+3,31)
    wcell3 = ws.cell(i+3,32)
    wcell4 = ws.cell(i+3,33)
    wcell5 = ws.cell(i+2,34)

    #State what value should be written to the cell
    wcell1.value = m_dot_list[i]
    wcell2.value = Qu_beta_list[i]
    wcell3.value = It_col_beta_list[i]
    wcell4.value = Tfo_beta_list[i]
    wcell5.value = Tfm_beta_list[i]
