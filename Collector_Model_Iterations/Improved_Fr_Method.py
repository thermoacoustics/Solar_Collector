# -*- coding: utf-8 -*-
"""
Created on Sat Mar 21 21:52:54 2020
...
def absorbed_radiation(Ib,Id,Pg,beta,theta_deg,t,a,Ac,Ta,m_dot,Fr0,P,Tfi_array,Ula,Ulb):
    ...
    #Calculate initial Fr estimate
    Fr = ((Fr0*Ula)/Ul)*((1-np.exp(-Ul))/(1-np.exp(-Ula)))
    ...
    for n in range(Z):
        ...
        #Calculate new Fr value with new Ul value
        Fr = ((Fr0*Ula)/Ul)*((1-np.exp(-Ul))/(1-np.exp(-Ula)))
    ...
    Function_outputs = [Qu,Tfo,eff,T_diff,Ul,T_grad,Tfm,Tpm,Fr]
    
    return Function_outputs

#Output an array of useful power results
Qu_array = absorbed_radiation(Ib,Id,Pg,beta,theta_deg,t,a,Ac,Ta,m_dot,Fr0,P,Tfi_array,Ula,Ulb)[0]
    
#Output an array of collector efficiency results
eff_array = absorbed_radiation(Ib,Id,Pg,beta,theta_deg,t,a,Ac,Ta,m_dot,Fr0,P,Tfi_array,Ula,Ulb)[2]

#Output an array of the results for the temperature difference between the mean fluid temperature and ambient temperature
T_diff_array = absorbed_radiation(Ib,Id,Pg,beta,theta_deg,t,a,Ac,Ta,m_dot,Fr0,P,Tfi_array,Ula,Ulb)[3]

#Output array for Ul results
Ul_array = absorbed_radiation(Ib,Id,Pg,beta,theta_deg,t,a,Ac,Ta,m_dot,Fr0,P,Tfi_array,Ula,Ulb)[4]

#Output array of plate to fluid temperature difference results
T_grad_array = absorbed_radiation(Ib,Id,Pg,beta,theta_deg,t,a,Ac,Ta,m_dot,Fr0,P,Tfi_array,Ula,Ulb)[5]

#Output array of fluid mean temperature
Tfm_array = absorbed_radiation(Ib,Id,Pg,beta,theta_deg,t,a,Ac,Ta,m_dot,Fr0,P,Tfi_array,Ula,Ulb)[6]

#Output array of plate mean temperature
Tpm_array = absorbed_radiation(Ib,Id,Pg,beta,theta_deg,t,a,Ac,Ta,m_dot,Fr0,P,Tfi_array,Ula,Ulb)[7]

#Output array of Fr
F_dash_array = absorbed_radiation(Ib,Id,Pg,beta,theta_deg,t,a,Ac,Ta,m_dot,Fr0,P,Tfi_array,Ula,Ulb)[8]
...
for i in range(len(eff_array)):
    ...
    wcell1 = ws.cell(i+2,66)
    wcell2 = ws.cell(i+2,67)
    wcell3 = ws.cell(i+2,68)
    wcell4 = ws.cell(i+2,69)
    wcell5 = ws.cell(i+2,70)
    wcell6 = ws.cell(i+2,71)
    wcell7 = ws.cell(i+2,72)
