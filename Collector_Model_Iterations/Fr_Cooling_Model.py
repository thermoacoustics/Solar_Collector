# -*- coding: utf-8 -*-
"""
Created on Thu Apr 16 13:08:10 2020
...
Nc = 10 #Input the number of collectors in the array
...
Li = 5 #Input inlet pipe length in metres
Lo = 5 #Input outlet pipe length in metres
Di = 13*(10**(-3)) #Input inlet pipe diameter in metres
Do = 13*(10**(-3)) #Input outlet pipe diameter in metres
Fr = 0.8742
Uh = 144 #Input the house heat loss coefficient
long = 15
long_st = 0
EHX = 1
...
#Define variable used to sum hourly cooling power requirements
Q_cool_sum = 0
...
#Define list to append monthly cooling values to
Q_cool_list = []
...
Ud = 1 #Input pipe loss coefficient
...
for v in range(24):
    ...
    def absorbed_radiation(I,Ibh,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Ud,Li,Lo,Di,Do,Uh):
    
        #Calculate total radiation incident on a flat surface
        #I = Ib+Id
        #print('I:',I)
        
        #Calculate the required cooling power
        if Ta > 24:
            Q_cool = Uh*(Ta-24)
        else:
            Q_cool = 0
        ...
        Cpc = CP.PropsSI('Cp0mass','T',Tfi+273,'P',P,'water')
        ...
        #Calculate initial F_dash estimate
        F_dash = ((m_dot*Cpc)/(Ac*Ul))*np.log(1/(1-(Ac*Ul*Fr)/(m_dot*Cpc)))
        ...
        #Calculate an initial estimate of the losses through the pipe inlet
        Lin = Ud*Ai*(Tfi-Ta)
        ...
        for n in range(Z):
            ...
            for aa in range(Z):
                ...
                #Use counter to inform the user on whether or not the iteration has converged to within 1%
                pp += 1
                if pp == Z:
                    print('Lin iteration has not converged to within 1%')
                
                #Calculate new collector inlet temperature estimate
                Tci = Tfi - Lin/(m_dot*Cpi)
                
                #Calculate new mean temperature through the inlet pipe
                Tim = (Tci+Tfi)/2
                
                #Calculate the inlet specific heat constant
                Cpi = CP.PropsSI('Cp0mass','T',Tim+273,'P',P,'water')
                
                #Introduce Lin_est so the value of Lin in this iteration can be compared to the values in the last iteration
                Lin_est = Lin
                
                #Calculate new losses through the pipe inlet
                Lin = Ud*Ai*(Tim-Ta)
                
                #Terminate loop if the new value of Lin is within 1% of the previous value
                #if np.all(np.logical_and((abs(Lin) > 0.99*abs(Lin_est)),(abs(Lin) < 1.01*abs(Lin_est)))):
                if abs(Lin) > 0.99*abs(Lin_est) and abs(Lin) < 1.01*abs(Lin_est):
                    break
                else:
                    continue
            ...
            Cpc = CP.PropsSI('Cp0mass','T',Tfm+273,'P',P,'water')
        
            #Calculate new Fr value with new Cp value
            #Fr = ((m_dot*Cp)/(Ac*Ul))*(1-np.exp(-(Ac*Ul*F_dash)/(m_dot*Cp)))
            
            #Calculate new F_dash value with new Cp value 
            F_dash = ((m_dot*Cpc)/(Ac*Ul))*np.log(1/(1-(Ac*Ul*Fr)/(m_dot*Cpc)))
        
            #Calculate Tpm estimate
            Tpm = Tci+(Qu/(Ac*Fr*Ul))*(1-Fr)
        ...
        Tco = Ta + (Sa/Ul) + (Tfi-Ta-(Sa/Ul))*np.exp(-(Ac*Ul*F_dash)/(m_dot*Cpc))
        #print('Tfo',Tfo)
        ...
        for ii in range(200):
            ...
            Tfo = Tco - Lout/(m_dot*Cpo)
            
            #Calculate new estimate of the mean temperature through the outlet pipe
            Tom = (Tco+Tfo)/2
            
            #Calculate outlet specific heat constant
            Cpo = CP.PropsSI('Cp0mass','T',Tom+273,'P',P,'water')
        ...
        #Calculate the flow rate flowing through the thermoacoustic system
        m_dot_s = m_dot*Nc
        ...
        if Ta <= 24:
            ...
            #Calculate initial estimate of the heat input required by the thermoacoustic system
            Qh = 3*(table_read(Ta,24,Tfo)[1])
            #print('Qh:',Qh)
            ...
            if Qh <= 0:
                ...
                #Ttm = 0
                Cpti = CP.PropsSI('Cp0mass','T',Tfo+273,'P',P,'water')
                Cptm = Cpti
            
                for zz in range(ZZ):
                    if zz == ZZ:
                        print('Cooling Output Iteration has not converged to within 1%')
                    
                    #Calculate new estimate of fluid temperature leaving the thermoacoustic system
                    Tfi = Tfo - (Qh/(EHX*m_dot_s*Cptm))
                    
                    #Calculate the mean temperature through the thermoacoustic system
                    Ttm = (Tfi+Tfo)/2
                    
                    Cptm = CP.PropsSI('Cp0mass','T',Ttm+273,'P',P,'water')
                
                    #print('Ttm:',Ttm)
                    
                    #Create variable so that the value of Qh in the current iteration can be compared to the value in the previous iteration
                    Qh_est = Qh
                    
                    #Calculate a new estimate of the heat input required by the thermoacoustic system
                    Qh = 3*(table_read(Ta,24,Ttm)[1])
                    
                    #Terminate loop if the new value of Lin is within 1% of the previous value
                    #if np.all(np.logical_and((abs(Lin) > 0.99*abs(Lin_est)),(abs(Lin) < 1.01*abs(Lin_est)))):
                    if abs(Qh) > 0.99*abs(Qh_est) and abs(Qh) < 1.01*abs(Qh_est):
                        break
                    else:
                        continue
                    ...
                    #print(Ttm)
                    #Calculate the cooling power provided
                Qc = 3*(table_read(Ta,24,Ttm)[0])
        ...
        Function_outputs = [Qu_loss, It_col, Q_cool, Qc]
        
        return Function_outputs

    #Output an array of useful power results
    Qu_array = absorbed_radiation(I,Ibh,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Ud,Li,Lo,Di,Do,Uh)[0]
    
    #Output an array of collector efficiency results
    #eff_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb)[2]

    #Output an array of the results for the temperature difference between the mean fluid temperature and ambient temperature
    #T_diff_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb)[3]

    #Output array for Ul results
    #Ul_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb)[4]

    #Output array of plate to fluid temperature difference results
    #T_grad_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb)[5]

    #Output array of fluid mean temperature
    #Tfm_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb)[2]

    #Output array of plate mean temperature
    #Tpm_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb)[7]

    #Output array of total incident radiation on the plate
    It_col_array = absorbed_radiation(I,Ibh,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Ud,Li,Lo,Di,Do,Uh)[1]

    #Output array of total beam radiation on the plate
    #Ibt_col_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb)[9]

    #Output array of total diffuse radiation on the plate
    #Idt_col_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb)[10]

    #Output array of total diffuse radiation on the plate
    #Igt_col_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb)[11]

    #Output array for the energy output for that hour
    #E_out_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb)[12]
    
    #Output array for angle of incidence for that hour
    #theta_deg_array = absorbed_radiation(I,Ibn,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb)[2]
    
    #Output an array of required cooling values
    Q_cool_array = absorbed_radiation(I,Ibh,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Ud,Li,Lo,Di,Do,Uh)[2]
    
    #Output an array of delivered cooling values
    Qc_array = absorbed_radiation(I,Ibh,Pg,beta,lat,n,h,t,a,Ac,Ag,Ta,m_dot,Fr,P,Tfi,Ula,Ulb,Ud,Li,Lo,Di,Do,Uh)[3]
    ...
    #Give each cooling requirement value in kilowatt hours
    Q_cool_kW = Q_cool_array/1000
    
    #Give each delivered cooling value in kilowatt hours
    Qc_kW = Qc_array/1000
    ...
    Qc_list.append(Qc_kW)
    
    Qu_tot += Qu_kW
    
    It_col_sum += It_col_kW
    
    Q_cool_sum += Q_cool_kW
...
#print(absorbed_radiation(Ib,Id,Pg,beta,theta_deg,t,a,Ac,Ul,Tfi,Ta,m_dot,Fr,P))
...
#for i in range(len(Qu_tot_list)):
    ...
    #wcell1 = ws.cell(i+3,35)
    #wcell2 = ws.cell(i+3,34)
    #wcell3 = ws.cell(i+3,33)
